...
from api.request import pokelist, pokesearch
from .data_processing import *
import json

search_list = []
for pos, pokemon in enumerate(pokelist):
    slug = pokemon["relative_name"].replace(' ', '-').replace('♀', '-female').replace('♂', '-male').lower()
    search_list.append(slug)
    pokesearch[pos]['url_name'] = slug
...
def pokemondetail(request, p):
    ...
    if p.lower() in search_list:
        i = search_list.index(p.lower())
        copy = pokelist[i].copy()
        copy['weight'] = pokelist[i]['weight'] / 10
        copy['height'] = pokelist[i]['height'] / 10
        copy['url_image'] = f'https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/{int(copy["id"])}.png'
        ...
        return render(request,'detail.html', {
            'pokemon': copy,
            'before': pokesearch[i-1],
            'later': pokesearch[later],
            'moves': moveslist,
